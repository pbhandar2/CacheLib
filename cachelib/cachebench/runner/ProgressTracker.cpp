...
BlockReplayProgressTracker::BlockReplayProgressTracker(const BlockCacheStressorBase& s,
                                 const std::string& detailedStatsFile)
    : stressor_(s) {
  if (!detailedStatsFile.empty()) {
    statsFile_.open(detailedStatsFile, std::ios::app);
...
BlockReplayProgressTracker::~BlockReplayProgressTracker() {
  try {
    if (statsFile_.is_open()) {
      statsFile_.close();
    }
...
void BlockReplayProgressTracker::work() {
  auto now = std::chrono::system_clock::now();
  auto nowTimeT = std::chrono::system_clock::to_time_t(now);
  char buf[16];
  struct tm time;
